function alertsDonut() {
  ...
  var svg = d3.select(".alerts-donut-graph").append("svg")
    .attr("width", width - 40)
    .attr("height", height)
    .append("g")
  ...
  d3.csv(window.location.origin+"/open-unopen-stats.csv", type, function(error, data) {
    ...
    g.append("text")
      .attr("id", "alerts_category_text")
      .text();

    g.append("text")
      .attr("y","1em")
      .attr("id", "alerts_num_text")
      .text();

    g.append("text")
      .attr("y","1em")
      .attr("id", "alerts_center_text")
      .text();

    var case_text = "Open"
    d3.select("#alerts_center_text")
      .html(case_text)
      .attr("x", -8.0*case_text.length/2)
      .attr("y",1.4*radius)
    d3.select("#alerts_category_text")
