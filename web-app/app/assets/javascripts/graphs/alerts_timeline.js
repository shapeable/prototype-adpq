...
//create an SVG
var svg = d3.select(".alerts-timeline-graph").append("svg")
...
function redraw() {
  ...
  thegraph.selectAll("circle")
    ...
    .append("circle")
      ...
      .on("mouseout", function(d){
        d3.select(this).style('opacity', 1e-6)//1e-6
      });
  ...
  legendUpdate.select("circle")
    .attr('cx', function(d, i){
      return width/3+legendscale(d.values[d.values.length-1].value);});
